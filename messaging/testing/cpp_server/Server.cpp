...
Server::Server(int iPort, int iPortDatagram, bool* pResult) {
	...
	if (bind(m_iListen, (struct sockaddr *) &m_addrMe, sizeof(struct sockaddr))
			== -1) {
...
bool Server::Connect() {
	...
	if ((m_iSock = accept(m_iListen, (struct sockaddr *) &m_addrRemote,
			&iSinSize)) == -1) {
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iLen; i++) {
			for (j = 0; j < sizeof(int); j++)
				ptr[i * sizeof(int) + j] = (char) valptr[(i + 1) * sizeof(int)
						- j - 1];
		}
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iLen; i++) {
			for (j = 0; j < sizeof(float); j++)
				ptr[i * sizeof(float) + j] = (char) valptr[(i + 1)
						* sizeof(float) - j - 1];
		}
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iLen; i++) {
			for (j = 0; j < sizeof(double); j++)
				ptr[i * sizeof(double) + j] = (char) valptr[(i + 1)
						* sizeof(double) - j - 1];
		}
...
// Send a file
bool Server::SendFile(const char* pFile) {
	char* pTemp = NULL;
	printf("pFile = %s\n", pFile);
	FILE *requestedFile = fopen(pFile, "r");
	if (pFile == NULL) {
		perror("Server::SendFile - Error opening file");
		return false;
	}
		...
		bool sent = SendInts(&fileSize, 1);

		pTemp = (char *) m_pBuffer;
		fread(pTemp, 1, fileSize, requestedFile);

		if (send(m_iSock, pTemp, fileSize, 0) == -1) {
			perror("Server::SendFile, send File");
			return false;
		}

		if (VERBOSE) {
			printf("Server: sending file of %d bytes - ", fileSize);
			printf("\n");
		}
...
int Server::RecvInts(int* pVals, int iLen) {
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iNum; i++) {
			for (j = 0; j < sizeof(int); j++)
				ptr[i * sizeof(int) + j] = (char) pResult[(i + 1) * sizeof(int)
						- j - 1];
		}
	} else {
...
int Server::RecvFloats(float* pVals, int iLen) {
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iNum; i++) {
			for (j = 0; j < sizeof(float); j++)
				ptr[i * sizeof(float) + j] = (char) pResult[(i + 1)
						* sizeof(float) - j - 1];
		}
	} else {
		// Leave the byte order as is
...
int Server::RecvDoubles(double* pVals, int iLen) {
	...
	if (m_bReverse) {
		...
		for (i = 0; i < iNum; i++) {
			for (j = 0; j < sizeof(double); j++)
				ptr[i * sizeof(double) + j] = (char) pResult[(i + 1)
						* sizeof(double) - j - 1];
		}
	} else {
		// Leave the byte order as is
