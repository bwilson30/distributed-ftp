...
// Modified by Brett Wilson for File Transfer
// Georgia Tech ECE 6102, bwilson30@gatech.edu
// September 2012
...
public class Server {
	...
	public void Connect() throws IOException {
		...
		if (VERBOSE)
			System.out.println("Client: opening socket to " + address
					+ " on port " + port + ", datagrams on port = " + dataport);
	...
	public void SendFile(String fileName) throws IOException {		
		...
		int buff[] = new int[1];
		buff[0] = fileSize;
		//SendString(String.valueOf(fileSize) + '^');
		SendInts(buff, 1);
		
		if (VERBOSE) {
			System.out.print("Client: sending file of " + fileSize + " bytes: ");
		}
		...
		output.write(fileByteArray, 0, fileByteArray.length);
		output.flush();
		...
		if (reverse) {
			...
			for (i = 0; i < len; i++) {
				for (j = 0; j < INT_SIZE; j++) {
					flip_array[(i + 1) * INT_SIZE - j - 1] = byte_array[i
							* INT_SIZE + j];
				}
			}
		...
		if (reverse) {
			...
			for (i = 0; i < len; i++) {
				for (j = 0; j < FLOAT_SIZE; j++) {
					flip_array[(i + 1) * FLOAT_SIZE - j - 1] = byte_array[i
							* FLOAT_SIZE + j];
				}
			}
		...
		if (reverse) {
			...
			for (i = 0; i < len; i++) {
				for (j = 0; j < DOUBLE_SIZE; j++) {
					flip_array[(i + 1) * DOUBLE_SIZE - j - 1] = byte_array[i
							* DOUBLE_SIZE + j];
				}
			}
	...
	public int RecvFile(String filePath, int maxlen) throws IOException {
		int i;
		int totalbytes = 0;
		int numbytes;

		if (maxlen > BUFFSIZE)
			System.out.println("Sending more bytes than will fit in buffer!");
	...
	public int RecvInts(int val[], int maxlen) throws IOException {
		 ...
		 * then convert to an array of ints, more fun than you can shake a stick
		 * at!
		 */
		...
		if (reverse) {
			...
			for (i = 0; i < totalbytes / INT_SIZE; i++) {
				for (j = 0; j < INT_SIZE; j++) {
					flip_array[(i + 1) * INT_SIZE - j - 1] = buff[i * INT_SIZE
							+ j];
				}
	...
	public int RecvDoubles(double val[], int maxlen) throws IOException {
		...
		if (reverse) {
			...
			for (i = 0; i < totalbytes / DOUBLE_SIZE; i++) {
				for (j = 0; j < DOUBLE_SIZE; j++) {
					flip_array[(i + 1) * DOUBLE_SIZE - j - 1] = buff[i
							* DOUBLE_SIZE + j];
				}
	...
	public int RecvFloats(float val[], int maxlen) throws IOException {
		...
		if (reverse) {
			...
			for (i = 0; i < totalbytes / FLOAT_SIZE; i++) {
				for (j = 0; j < FLOAT_SIZE; j++) {
					flip_array[(i + 1) * FLOAT_SIZE - j - 1] = buff[i
							* FLOAT_SIZE + j];
				}
