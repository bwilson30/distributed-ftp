...
class Client {
public:
	Client(int iPort, int iPortDatagram, const char* pStrHost, bool bReverse,
			bool* pResult);
	~Client();
	...
	bool SendFile(const char* pFile);					// Send a file
	...
	int RecvFile(const char* pFilePath, int iLen);		// Receive a file
