...
Client::Client(int iPort, int iPortDatagram, const char* pStrHost,
	...
	if (connect(m_iSock, (struct sockaddr *) &m_addrRemote,
...
// Send a file
bool Client::SendFile(const char* pFile) {
	char* pTemp = NULL;
	int fileSize = 0;
	printf("pFile = %s\n", pFile);
	...
	if (requestedFile == NULL) {
		SendInts(&fileSize, 1);
		perror("Client::SendFile - Error opening file");
		return false;
	}
		...
		if (send(m_iSock, pTemp, fileSize, 0) == -1) {
			perror("Client::SendFile, send File");
			return false;
		}

		if (VERBOSE) {
			printf("Client: sending file of %d bytes - ", fileSize);
			printf("\n");
		}
